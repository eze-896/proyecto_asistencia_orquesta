...
namespace GUI_Login.control
{
    public class ControlProfesor
    {
        ...
        private static readonly Regex SoloLetrasRegex = new(@"^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$", RegexOptions.Compiled);
        ...
        private static bool ValidarProfesor(Profesor profesor)
        {
            // Validar DNI
            ...
            if (string.IsNullOrWhiteSpace(profesor.Nombre) || profesor.Nombre.Length > 25 ||
                !SoloLetrasRegex.IsMatch(profesor.Nombre))
            {
            ...
            if (string.IsNullOrWhiteSpace(profesor.Apellido) || profesor.Apellido.Length > 25 ||
                !SoloLetrasRegex.IsMatch(profesor.Apellido))
            {
            ...
            if (string.IsNullOrWhiteSpace(profesor.Email) || profesor.Email.Length > 50 ||
                !EmailRegex.IsMatch(profesor.Email))
            {
        ...
        public static bool ValidarCamposObligatorios(string nombre, string apellido, string dni, string telefono, string email, int? idInstrumento)
        {
